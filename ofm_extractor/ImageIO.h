...
public:
	...
	template <class T>
	static bool loadImageFromMat(cv::Mat inputmat, T*& pImagePlane, int& width, int& height, int& nchannels);
	template <class T>
...
template <class T>
bool ImageIO::loadImageFromMat(cv::Mat inputmat, T *&pImagePlane, int &width, int &height, int &nchannels)
{
