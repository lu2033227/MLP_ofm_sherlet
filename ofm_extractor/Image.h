...
public:
	...
	virtual bool imreadfromMat(cv::Mat inputmat);
...
template <class T>
bool Image<T>::imreadfromMat(cv::Mat inputmat)
{
	clear();
	if (ImageIO::loadImageFromMat(inputmat, pData, imWidth, imHeight, nChannels))
	{
		computeDimension();
		colorType = BGR; // when we use qt or opencv to load the image, it's often BGR
		return true;
