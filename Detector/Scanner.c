...
/** allocates arays */
void bbf_Scanner_alloc( struct bbf_Scanner* ptrA )
{
	...
	bbs_UInt16Arr_create( &ptrA->workImageE, ( ( woL >> 1 ) + ( woL & 1 ) ) * hoL );
	bbs_UInt16Arr_fill( &ptrA->workImageE, 0 );

	/* allocate bit image */
	bim_UInt32Image_create( &ptrA->bitImageE, xwoL, ( xhoL >> 5 ) + ( ( ( xhoL & 0x1F ) != 0 ) ? 1 : 0 ) );
	bim_UInt32Image_setAllPixels( &ptrA->bitImageE, 0, 0 );

	/* allocate patch buffer */
	bbs_UInt32Arr_create( &ptrA->patchBufferE, ptrA->bitImageE.widthE );
	bbs_UInt32Arr_fill( &ptrA->patchBufferE, 0 );

	/* allocate line buffer */
	bbs_UInt16Arr_create( &ptrA->lineBufE, woL + ( woL & 1 ) );

	/* allocate table */
	bim_UInt32Image_create( &ptrA->satE, woL + pwoL, pwoL + 1 );

	/* allocate buffers */
	bbs_UInt32Arr_create( &ptrA->idxArrE, ptrA->bufferSizeE );
	bbs_Int32Arr_create(  &ptrA->actArrE, ptrA->bufferSizeE );

	bbs_Int32Arr_create(  &ptrA->outArrE, ptrA->bufferSizeE >> 1 );
}

/* ------------------------------------------------------------------------- */

/** downscales work image */
void bbf_Scanner_downscale( struct bbf_Scanner* ptrA )
{
 ...
 */
void bbf_Scanner_copyImage( struct bbf_Scanner* ptrA, const void* imagePtrA, uint32 imageWidthA, uint32 imageHeightA, const struct bts_Int16Rect* roiPtrA )
{
	if( imageWidthA > ptrA->maxImageWidthE || imageHeightA > ptrA->maxImageHeightE ) 
	{
		printf( "Input image (%ix%i)is too large; Scanner is configured for maximal (%ix%i)", 
...
/** creates bit image */
void bbf_Scanner_createBitImage( struct bbf_Scanner* ptrA )
{
	/* declarations */
	...
	bim_UInt32Image_size( &ptrA->bitImageE, xwoL, ( xhoL >> 5 ) + ( ( ( xhoL & 0x1F ) != 0 ) ? 1 : 0 ) );
	bim_UInt32Image_setAllPixels( &ptrA->bitImageE, 0, 0 );
	...
	bim_UInt32Image_size( &ptrA->satE, wsL, hsL );
...
void bbf_Scanner_initPatchBuffer( struct bbf_Scanner* ptrA )
{
	bbs_UInt32Arr_size( &ptrA->patchBufferE, ptrA->bitImageE.widthE );	
...
void bbf_Scanner_init( struct bbf_Scanner* ptrA )
{
	ptrA->scaleExpE = 0;
	ptrA->scaleE = 0;
	ptrA->xE = 0;
	...
	ptrA->workWidthE = 0;
	ptrA->workHeightE = 0;
	bbf_BitParam_init( &ptrA->bitParamE );
	bbs_UInt16Arr_init( &ptrA->workImageE );
	bim_UInt32Image_init( &ptrA->satE );
	bim_UInt32Image_init( &ptrA->bitImageE );
	bbs_UInt32Arr_init( &ptrA->patchBufferE );
	bbs_UInt16Arr_init( &ptrA->lineBufE );

	bbs_UInt32Arr_init( &ptrA->idxArrE );
	bbs_Int32Arr_init( &ptrA->actArrE );
	bbs_Int32Arr_init( &ptrA->outArrE );
	ptrA->outCountE = 0;
	ptrA->intCountE = 0;
...
void bbf_Scanner_exit( struct bbf_Scanner* ptrA )
{
	ptrA->scaleExpE = 0;
	ptrA->scaleE = 0;
	ptrA->xE = 0;
	...
	ptrA->workWidthE = 0;
	ptrA->workHeightE = 0;
	bbf_BitParam_exit( &ptrA->bitParamE );
	bbs_UInt16Arr_exit( &ptrA->workImageE );
	bim_UInt32Image_exit( &ptrA->satE );
	bim_UInt32Image_exit( &ptrA->bitImageE );
	bbs_UInt32Arr_exit( &ptrA->patchBufferE );
	bbs_UInt16Arr_exit( &ptrA->lineBufE );

	bbs_UInt32Arr_exit( &ptrA->idxArrE );
	bbs_Int32Arr_exit( &ptrA->actArrE );
	bbs_Int32Arr_exit( &ptrA->outArrE );
	ptrA->outCountE = 0;
	ptrA->intCountE = 0;
	ptrA->bufferSizeE = 1024;
...
void bbf_Scanner_create( struct bbf_Scanner* ptrA, 
						 ...
						 uint32 bufferSizeA )
{
	...
	bbf_Scanner_alloc( ptrA );
}

/* ------------------------------------------------------------------------- */
	
void bbf_Scanner_bitParam( struct bbf_Scanner* ptrA,
						   const struct bbf_BitParam* bitParamPtrA )
{
	if( !bbf_BitParam_equal( &ptrA->bitParamE, bitParamPtrA ) )
	{
		bbf_BitParam_copy( &ptrA->bitParamE, bitParamPtrA );
		bbf_Scanner_createBitImage( ptrA );
	}

	bbf_Scanner_resetScan( ptrA );
}
...
void bbf_Scanner_resetScan( struct bbf_Scanner* ptrA )
{
	ptrA->xE = 0;
	ptrA->yE = 0;
	bbf_Scanner_initPatchBuffer( ptrA );
}

/* ------------------------------------------------------------------------- */

void bbf_Scanner_assign( struct bbf_Scanner* ptrA,
...
{
	/* copy image */
	bbf_Scanner_copyImage( ptrA, imagePtrA, imageWidthA, imageHeightA, roiPtrA );

	ptrA->scaleE = ptrA->minScaleE;
	bbf_BitParam_copy( &ptrA->bitParamE, paramPtrA );
	...
	/* compute effective max scale */
	{
		...
		if( ptrA->maxScaleE > 0 ) 
			ptrA->effMaxScaleE = ptrA->effMaxScaleE < ptrA->maxScaleE ? ptrA->effMaxScaleE : ptrA->maxScaleE;
	}

	ptrA->scaleExpE = 0;

	/* downscale work image if necessary */
	while( ptrA->scaleE > ( ( uint32 )( 2 << ptrA->scaleExpE ) << 20 ) ) 
		bbf_Scanner_downscale( ptrA );
...
flag bbf_Scanner_nextScale( struct bbf_Scanner* ptrA )
{
	if( ptrA->scaleE + bbf_Scanner_scalePrd( ptrA->scaleE, ptrA->scaleStepE ) >= ptrA->effMaxScaleE ) 
		return FALSE;
...
flag bbf_Scanner_next( struct bbf_Scanner* ptrA )
{
...
void bbf_Scanner_goToXY( struct bbf_Scanner* ptrA, int32 xA, int32 yA )
{
...
void bbf_Scanner_goToIndex( struct bbf_Scanner* ptrA, uint32 scanIndexA )
{
	int32 yL = scanIndexA / ptrA->currentWidthE; 
	int32 xL = scanIndexA - yL * ptrA->currentWidthE;
	bbf_Scanner_goToXY( ptrA, xL, yL ); 
}

/* ------------------------------------------------------------------------- */

void bbf_Scanner_goToUls( struct bbf_Scanner* ptrA, 
...
void bbf_Scanner_resetOutPos( struct bbf_Scanner* ptrA ) 
{
	ptrA->outCountE = 0;
}
...
void bbf_Scanner_resetIntPos( struct bbf_Scanner* ptrA )
{
	ptrA->intCountE = 0;
}
...
void bbf_Scanner_addIntPos( struct bbf_Scanner* ptrA,
							uint32 idxA,
							int32 actA )
{
...
void bbf_Scanner_addOutPos( struct bbf_Scanner* ptrA, 
							int32 xA, 
							int32 yA, 
							uint32 scaleA, 
							int32 actA )
...
/* remove overlaps */
uint32 bbf_Scanner_removeOutOverlaps( struct bbf_Scanner* ptrA,
									  uint32 overlapThrA )
{
...
uint32 bbf_Scanner_removeIntOverlaps( struct bbf_Scanner* ptrA,
									  uint32 overlapThrA )
{
    uint32 begIdxL = 0;		 /* begin index */
