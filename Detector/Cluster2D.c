...
struct bts_Flt16Vec2D bts_Cluster2D_center( const struct bts_Cluster2D* ptrA )
{
	...
	if( ptrA->sizeE == 0 ) 
		return bts_Flt16Vec2D_create16( 0, 0, 0 );
						   ...
						   uint32 sizeA )
{
	...
	ptrA->vecArrE = malloc( sizeA * sizeof( struct bts_Int16Vec2D ) );
	ptrA->sizeE = sizeA;
								  ...
								  uint32 dstBbpA )
{
	...
	{
		printf( "void bts_Cluster2D_copyTransform( struct bts_Cluster2D* ptrA, const struct bts_Cluster2D* srcPtrA, struct bts_Flt16Alt2D altA, uint32 dstBbpA ): allocated size too low in destination cluster" );
		return;
	}
							  ...
							  const uint16* memPtrA )
{
	...
	memPtrA += bbs_memReadVersion32( &versionL, bts_CLUSTER2D_VERSION, memPtrA );
	memPtrA += bbs_memRead32( &sizeL, memPtrA );
	memPtrA += bbs_memRead32( &ptrA->bbpE, memPtrA );

	if( ptrA->allocatedSizeE < sizeL )
	{
		bts_Cluster2D_create( ptrA, sizeL );
	}
	else
	{
		bts_Cluster2D_size( ptrA, sizeL );
	}

	memPtrA += bbs_memRead16Arr( ptrA->vecArrE, ptrA->sizeE * 2, memPtrA );

	if( memSizeL != bts_Cluster2D_memSize( ptrA ) )
	{
		printf( "uint32 bts_Cluster2D_memRead( const struct bts_Cluster2D* ptrA, const void* memPtrA ):\n"
                   "size mismatch" ); 
		return 0;
	}
	return memSizeL;
										 ...
										 enum bts_AltType altTypeA )
{
	...
	{
		printf( "struct bts_Flt16Alt2D bts_Cluster2D_alt( ... ):\n"
	...
	cpL = bts_Cluster2D_center( srcPtrA );
	cqL = bts_Cluster2D_center( dstPtrA );
